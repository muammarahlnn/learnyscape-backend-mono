...
import (
	"context"
	"fmt"
	authtoken "learnyscape-backend-mono/internal/domain/auth/constant"
...
type ForgotPasswordConsumer struct {
...
func NewForgotPasswordConsumer(conn *amqp.Connection, mailer smtputil.Mailer) mq.AMQPConsumer {
	queue := authtoken.ForgotPasswordQueue
	key := authtoken.ForgotPasswordKey
	exchange := authtoken.ForgotPasswordExchange
...
func (c *ForgotPasswordConsumer) Consume(ctx context.Context, nWorker int) error {
	for i := 1; i <= nWorker; i++ {
		c.wg.Add(1)
		go c.Start(ctx, i)
	}
...
func (c *ForgotPasswordConsumer) Start(ctx context.Context, workerID int) {
	defer c.wg.Done()
...
func (c *ForgotPasswordConsumer) Handler() mq.AMQPHandler {
	return func(ctx context.Context, body []byte) error {
		var event dto.ForgotPasswordEvent
		...
		return c.mailer.SendMail(
			ctx,
			event.Email,
			constant.ResetPasswordSubject,
			fmt.Sprintf(constant.ResetPasswordTemplate, event.Name, event.Token),
		)
	}
}

func (c *ForgotPasswordConsumer) Queue() string {
	return c.queue
}

func (c *ForgotPasswordConsumer) Close() error {
